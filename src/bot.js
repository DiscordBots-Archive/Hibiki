...
Hibiki
    ...
    .on('message', async (msg) => {
        if (!msg.guild || msg.author.bot || !msg.guild.settings.get('antiInvite')) return;
        ...
        if (!earnedRecently.includes(msg.author.id)) {
            ...
            await Hibiki.redis.setAsync(`points${msg.author.id}`, moneyEarned);
        ...
        if (!gainedXPRecently.includes(msg.author.id)) {
            ...
            Experience.addExperience(msg.author.id, xpEarned).then(async () => {
                ...
                if (newLevel > oldLevel) {
                    ...
                    await Hibiki.redis.setAsync(`points${msg.author.id}`, newLevel);
                }
    ...
    .on('commandError', (cmd, err) => {
        ...
        if (err.message === 'Cannot send an empty message') return;

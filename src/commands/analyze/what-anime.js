...
const { base64 } = require('../../modules/utilities');
...
module.exports = class WhatAnimeCommand extends Command {
    ...
    async run(msg, { screenshot }) {
        try {
            ...
            const { body } = await get(screenshot);
            ...
            if (result.nsfw && !msg.channel.nsfw) return msg.reply('This is from a hentai, and this isn\'t an NSFW channel, pervert.');
            return msg.reply(
    ...
    async fetchRateLimit() {
        try {
            const { body } = await get('https://whatanime.ga/api/me')
    ...
    async search(file) {
        ...
        const { body } = await post('https://whatanime.ga/api/search')
            .query({ token: WHATANIME_KEY })
    ...
    async fetchPreview(data) {
        try {
            const { body } = await get('https://whatanime.ga/preview.php')
                .query({
